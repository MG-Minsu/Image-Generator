...
with tab1:
    ...
    if srt_for_audio is not None:
        try:
            ...
            if subtitles:
                ...
                if st.button("📋 Use Extracted Text", width=True, key="use_extracted_text"):
                    audio_text = all_text
    ...
    if audio_text.strip():
        ...
        if st.button("🚀 Generate Audio", type="primary", width=True, key="generate_audio_btn"):
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            with col_gen1:
                generate_all = st.button("🚀 Generate All Images", type="primary", width=True, key="gen_all_img")
            
            with col_gen2:
                if len(scene_descriptions) > 5:
                    generate_sample = st.button("🎯 Generate 5 Sample Images", width=True, key="gen_sample_img")
                else:
...
with tab3:
    ...
    with col1:
        ...
        if 'generated_audio' in st.session_state:
            if st.button("🎤 Use Generated Audio", width=True, key="use_generated_audio"):
    ...
    if 'image_data_for_download' in st.session_state and st.session_state.image_data_for_download:
        ...
        if st.button("📦 Create ZIP from Generated Images", width=True, key="create_zip_from_generated"):
    ...
    if st.button("🚀 Create Video", type="primary", width=True, key="generate_video_btn"):
        if not audio_file:
